...
Rx correlation (magnitude, phase, NxN heatmap + table), and basic sensitivity chart.
"""
...
# ------------------------------------------------------------------ #
# G. Basic Sensitivity Chart (Signed S_x)
# ------------------------------------------------------------------ #
def plot_basic_sensi_chart(sensitivity_data: dict, target_metric: str = "SE"):
    """
    Horizontal diverging bar chart of parameter impact on the chosen metric.
    Parameters are sorted by absolute sensitivity (descending) — most impactful at top.
    """
    _METRIC_DISPLAY = {
    ...
    # Colour: positive impact → cyan; negative impact → pink
    bar_colors = [COLOR_CYAN if v >= 0 else COLOR_PINK for v in vals]
    ...
    fig.add_trace(go.Bar(
        ...
        text=[f"S = {v:+.4f}  {rank_labels[i]}" for i, v in enumerate(vals)],
        textposition="outside",
        textfont=dict(size=10, color=COLOR_SECONDARY),
        hovertemplate="<b>%{y}</b><br>S = %{x:+.4f}<br>Normalized sensitivity coefficient:<br>S = (∂y/∂x)(x/y)<br><br>Higher magnitude indicates stronger influence.<extra></extra>",
        name=target_metric,
    ))
    ...
    fig.update_layout(
        template=_TEMPLATE,
        title=dict(
            text=f"Basic Sensitivity Chart — Impact on {metric_label}",
        ...
        paper_bgcolor="rgba(0,0,0,0)",
        xaxis=dict(
            title="Sensitivity Coefficient S_x",
    ...
    fig.add_vline(x=0, line_color="rgba(255,255,255,0.2)", line_width=1.5)
    return fig


# ------------------------------------------------------------------ #
# G2. Horizontal Bar Chart for Absolute Sensitivity
# ------------------------------------------------------------------ #
def plot_sensitivity_bar(sensitivity_data: dict, target_metric: str = "SE"):
    """
    Horizontal bar plot of absolute sensitivity coefficients sorted in descending order.
    """
    _METRIC_DISPLAY = {
        "SE":           "Spectral Efficiency (bits/s/Hz)",
        "BER":          "Bit Error Rate",
    ...
    sorted_d  = sorted(data, key=lambda x: abs(x[1]), reverse=True)
    names     = [x[0] for x in sorted_d]
    abs_vals  = [abs(x[1]) for x in sorted_d]
    vals      = [x[1] for x in sorted_d]
    ...
    fig.add_trace(go.Bar(
        ...
        orientation="h",
        marker_color=bar_colors,
        text=[f"|S| = {v:.4f}" for v in abs_vals],
        textposition="outside",
        textfont=dict(size=10, color=COLOR_SECONDARY),
        hovertemplate="<b>%{y}</b><br>|S| = %{x:.4f}<br>Normalized sensitivity coefficient:<br>S = (∂y/∂x)(x/y)<br><br>Higher magnitude indicates stronger influence.<extra></extra>",
    ...
    fig.update_layout(
        template=_TEMPLATE,
        title=dict(
            text=f"Absolute Parameter Importance — {metric_label}",
            font=dict(size=16, color=COLOR_PRIMARY),
        ),
        height=max(380, 50 * len(names) + 80),
        margin=dict(l=110, r=120, t=60, b=50),
        ...
        paper_bgcolor="rgba(0,0,0,0)",
        xaxis=dict(
            title="Absolute Sensitivity Coefficient |S_x|",
            gridcolor="rgba(255,255,255,0.08)",
            zerolinecolor="rgba(255,255,255,0.2)",
        ),
        yaxis=dict(
...
# ------------------------------------------------------------------ #
# G3. Sensitivity Heatmap (Inputs vs Outputs)
# ------------------------------------------------------------------ #
def plot_sensitivity_heatmap(sensitivity_data: dict):
    """
    Heatmap of input parameters vs output metrics showing signed sensitivities.
    """
    metrics = ["SE", "BER", "capacity_mbps", "y_power"]
    metric_labels = ["SE", "BER", "Capacity", "Rx Power"]

    # Use first available metric to read the parameter list
    first_metric_data = sensitivity_data.get(metrics[0], [])
    if not first_metric_data:
        # Fallback to whatever is available
        available_keys = list(sensitivity_data.keys())
        if available_keys:
            first_metric_data = sensitivity_data.get(available_keys[0], [])
        else:
            fig = go.Figure()
            _base_layout(fig, "No sensitivity data for Heatmap")
            return fig

    param_labels = [x[0] for x in first_metric_data]

    z_data = []
    for param in param_labels:
        row = []
        for metric in metrics:
            metric_data = sensitivity_data.get(metric, [])
            val = 0.0
            for p_label, s_val in metric_data:
                if p_label == param:
                    val = s_val
                    break
            row.append(val)
    ...
    fig = go.Figure(go.Heatmap(
        ...
        colorbar=dict(title=dict(text="Sensitivity S_x", font=dict(color=COLOR_SECONDARY)),
                      tickfont=dict(color=COLOR_SECONDARY)),
        hovertemplate="<b>Input:</b> %{y}<br><b>Output:</b> %{x}<br><b>S_x:</b> %{z:+.4f}<br>Normalized sensitivity coefficient:<br>S = (∂y/∂x)(x/y)<br><br>Higher magnitude indicates stronger influence.<extra></extra>"
    ))

    fig.update_layout(
        template=_TEMPLATE,
        title=dict(
            text="Sensitivity Matrix (Inputs vs Outputs)",
            font=dict(size=16, color=COLOR_PRIMARY),
        ),
        height=480,
...
# ------------------------------------------------------------------ #
# G4. Sensitivity Radar Chart
# ------------------------------------------------------------------ #
def plot_sensitivity_radar(sensitivity_data: dict):
    """
    Radar chart comparing absolute sensitivities |S_i| across outputs.
    """
    metrics = ["SE", "BER", "capacity_mbps", "y_power"]
    metric_labels = ["SE", "BER", "Capacity", "Rx Power"]
    colors = [COLOR_CYAN, COLOR_PINK, COLOR_GREEN, COLOR_YELLOW]
    ...
    if not first_metric_data:
        ...
        if available_keys:
            ...
            fig = go.Figure()
            _base_layout(fig, "No sensitivity data for Radar")
            return fig
    ...
    for metric, m_label, color in zip(metrics, metric_labels, colors):
        metric_data = sensitivity_data.get(metric, [])
        vals = []
        for param in param_labels:
            val = 0.0
        ...
        r_vals = vals + [vals[0]]
        theta_vals = param_labels + [param_labels[0]]
        ...
        fig.add_trace(go.Scatterpolar(
            ...
            hovertemplate="<b>%{theta}</b><br>" + m_label + " |S_x| = %{r:.4f}<extra></extra>"
        ))

    fig.update_layout(
        template=_TEMPLATE,
        title=dict(
            text="Sensitivity Radar (Cross-Metric Absolute Sensitivity comparison)",
            font=dict(size=16, color=COLOR_PRIMARY),
        ),
        polar=dict(
            bgcolor="rgba(0,0,0,0)",
            radialaxis=dict(visible=True, gridcolor="rgba(255,255,255,0.08)", tickfont=dict(size=10, color=COLOR_SECONDARY)),
            angularaxis=dict(gridcolor="rgba(255,255,255,0.08)", tickfont=dict(size=11, color=COLOR_PRIMARY)),
        ),
        font=dict(family="Inter, sans-serif", color=COLOR_SECONDARY),
        plot_bgcolor="rgba(0,0,0,0)",
        paper_bgcolor="rgba(0,0,0,0)",
...
# ------------------------------------------------------------------ #
# H. Radial Bar Chart for Sensitivity (sidebar preview card)
# ------------------------------------------------------------------ #
def plot_sensitivity_radial(sensitivity_data: dict, target_metric="SE"):
    ...
    names       = [x[0] for x in sorted_d]
    raw_vals    = [x[1] for x in sorted_d]
    ...
    fig.add_trace(go.Barpolar(
        ...
        opacity=0.9,
        customdata=raw_vals,
        hovertemplate="<b>%{theta}</b><br>Relative Impact: %{r:.1f}%<br>Coefficient S_x: %{customdata:+.4f}<extra></extra>"
    ))
